...
the expected template that I wanted, with Mocks, InjectMocks, focusing on testing the areas that I considered it makes
sense, and test coverage, which is also kind of good, but not really production grade. Would be a point in future work.\
...
&nbsp;&nbsp;&nbsp;&nbsp; Did some general handling of exceptions, which also depends on business requirements and can be
even more developed.\
&nbsp;&nbsp;&nbsp;&nbsp; Added Swagger in order to have a valid client in order for end-to-end testing.\
&nbsp;&nbsp;&nbsp;&nbsp; To be fair, I thought that this project would be much lighter, but in the end if you want to do
the things well done, sometimes it takes time.
...
Redis work.\
- Would get rid of hardcoded parts and create more static constants for them and also take out the dynamic config values 
to container level, so it's more flexible in terms of deploying different configs.
- Would do even more end-to-end testing and polishing